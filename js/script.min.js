...
function insertEvents(a, b) {
    for (var c = new Date(), d = c.getFullYear(), e = c.getMonth(), f = new Date(d, e + 1, 0), g = 0; g < b; g++) {
        var h = a[g][0].substring(a[g][0].search(c.toLocaleString("en", {
            month: "long"
        })) + c.toLocaleString("en", {
            month: "long"
        }).length, a[g][0].lastIndexOf(","));
        if (h >= c.getDate() - c.getDay() && h <= c.getDate() - c.getDay() + 6 || h <= f.getDate() - f.getDay() && h >= f.getDate()) {
            ...
            m.innerText = a[g][3], i.appendChild(m);
            document.querySelectorAll("div#campusEvents")[0].children[0].children[2].appendChild(i);
        }
    }
}
