function getEvents() {
    ...
    c.send(null), c.onreadystatechange = function() {
        if (4 == c.readyState) {
            ...
            localStorage.setObj("event_updated", j.getMonth() + 1 + "/" + j.getDate()), checkLocalStorage();
        }
    };
}

function insertEvents(a, b) {
    for (var c = new Date(), d = (c.getFullYear(), c.getMonth(), /[a-zA-Z]\-[A-Z|0-9][0-9]+/), e = 0; e < b; e++) {
        var f = a[e][0].substring(a[e][0].search(c.toLocaleString("en", {
            month: "long"
        })) + c.toLocaleString("en", {
            month: "long"
        }).length, a[e][0].lastIndexOf(","));
        if (f >= c.getDate() && f <= c.getDate() + 14) {
            ...
            l.innerText = a[e][3];
            var m = document.createElement("a"), n = a[e][3].replace(/ /g, ""), o = a[e][3].split(", ");
            if (1 == o[0].includes("(")) {
                var p = o[0].search("\\("), q = o[0].search("\\)");
                o[0] = o[0].replace(o[0].substr(p - 1, q), "");
            }
            var r = o[0].split(" "), s = !1;
            if (1 == d.test(n)) {
                m.setAttribute("onclick", "closeTab('campusEvents');"), m.setAttribute("href", "#" + d.exec(n)[0].replace("-", "")),
                m.appendChild(i), m.appendChild(j), m.appendChild(k), m.appendChild(l), g.appendChild(m),
                h.appendChild(g), 1 == d.exec(n)[0].includes("C-G") ? g.id = "C3" : "C" == d.exec(n)[0].substr(0, 1) && parseInt(d.exec(n)[0].substr(2, 4)) > 1300 && parseInt(d.exec(n)[0].substr(2, 4)) < 2e3 || "C" == d.exec(n)[0].substr(0, 1) && parseInt(d.exec(n)[0].substr(2, 4)) > 2300 && parseInt(d.exec(n)[0].substr(2, 4)) < 3e3 ? g.id = "C3" : g.id = d.exec(n)[0].substr(0, 1);
                var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                t.appendChild(h);
            } else {
                var u, v;
                for (var w in roomNames) for (var x in roomNames[w]) for (var y in roomNames[w][x]) r.length >= 2 && 1 == roomNames[w][x][y].name.includes(r[0].toLowerCase() + "-" + r[1].toLowerCase()) ? (s = !0,
                u = roomNames[w][x][y].name, v = w.toUpperCase()) : 1 == r.length && 1 == roomNames[w][x][y].name.includes(r[0].toLowerCase()) && (s = !0,
                u = roomNames[w][x][y].name, v = w.toUpperCase());
                if (1 == s) {
                    m.setAttribute("onclick", "closeTab('campusEvents');"), m.setAttribute("href", "#" + u),
                    g.id = v, m.appendChild(i), m.appendChild(j), m.appendChild(k), m.appendChild(l),
                    g.appendChild(m), h.appendChild(g);
                    var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                    t.appendChild(h);
                } else if (o.length > 1 && 1 == o[1].includes("Building") || o.length > 1 && 1 == o[0].includes("Building")) if (1 == o[1].includes("Building")) {
                    m.setAttribute("onclick", "closeTab('campusEvents');"), m.setAttribute("href", "#" + o[1].substr(o[1].search("Building") + 9)),
                    g.id = o[1].substr(o[1].search("Building") + 9), m.appendChild(i), m.appendChild(j),
                    m.appendChild(k), m.appendChild(l), g.appendChild(m), h.appendChild(g);
                    var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                    t.appendChild(h);
                } else {
                    var z = /[A-Z|0-9][0-9]+/;
                    m.setAttribute("onclick", "closeTab('campusEvents');"), m.setAttribute("href", "#" + o[0].substr(o[0].search("Building") + 9) + z.exec(o[1])),
                    g.id = o[0].substr(o[0].search("Building") + 9), m.appendChild(i), m.appendChild(j),
                    m.appendChild(k), m.appendChild(l), g.appendChild(m), h.appendChild(g);
                    var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                    t.appendChild(h);
                } else if (2 == r.length && 1 == r[0].includes("Building") && 1 == r[1].length) {
                    m.setAttribute("onclick", "closeTab('campusEvents');"), m.setAttribute("href", "#" + r[1]),
                    g.id = r[1], m.appendChild(i), m.appendChild(j), m.appendChild(k), m.appendChild(l),
                    g.appendChild(m), h.appendChild(g);
                    var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                    t.appendChild(h);
                } else if (1 == a[e][3].includes("Student Center")) {
                    m.setAttribute("onclick", "closeTab('campusEvents');"), m.setAttribute("href", "#E"),
                    g.id = "E", m.appendChild(i), m.appendChild(j), m.appendChild(k), m.appendChild(l),
                    g.appendChild(m), h.appendChild(g);
                    var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                    t.appendChild(h);
                } else {
                    g.appendChild(i), g.appendChild(j), g.appendChild(k), g.appendChild(l), h.appendChild(g);
                    var t = document.querySelectorAll("div#campusEvents")[0].children[0].children[2];
                    t.appendChild(h);
                }
            }
...
function checkLocalStorage() {
    ...
    var a = navigator.onLine, b = new Date();
    b.getFullYear(), b.getMonth();
    1 == a ? null == localStorage.getObj("event_key") || null == localStorage.getObj("eventNumber_key") ? getEvents() : localStorage.getObj("event_updated") != b.getMonth() + 1 + "/" + b.getDate() ? getEvents() : insertEvents(localStorage.getObj("event_key"), localStorage.getObj("eventNumber_key")) : null == localStorage.getObj("event_key") || null == localStorage.getObj("eventNumber_key") ? console.log("No connection to retrieve events") : insertEvents(localStorage.getObj("event_key"), localStorage.getObj("eventNumber_key"));
}
...
function switchLegend() {
    "block" == document.getElementById("bLegend").style.display && document.getElementById("buildingLegendSwitch").click();
}

function safetyMode() {
    var a = document.getElementById("Safety");
    "block" === a.style.display ? a.style.display = "none" : a.style.display = "block";
}

function safetyOff() {
    "none" !== document.getElementById("Safety").style.display && document.getElementById("safetySwitch").click();
}
            ...
            m = {
                ...
                init: function(a) {
                    ...
                    this.hammer = Hammer(a.svgElement, {
                        ...
                        if (toggleTab(a, "helpbox"), toggleTab(a, "devbox"), toggleTab(a, "campusEvents"),
                        "Campus" === b) {
...
function searchRoomNumber() {
    ...
    if (console.log("building = " + a.building), "campus" === a.roomName.toLowerCase()) document.getElementById("roomSearch").value = "",
    changeFloor("Campus"); else {
        ...
        if ("" !== a.building && (c = a.building), "" !== a.room && (b = a.room), "" !== a.roomName) {
            ...
            switch (function() {
              ...
              case "3":
                "4" === b[1] ? (f = "3", g = "4") : "3" === b[1] ? (f = "3", g = "3") : "2" === b[1] ? (f = "3",
                g = "2") : (f = "3", g = "1");
                break;
...
function changeFloor(a, b) {
    ...
    var c = a.toUpperCase();
    document.getElementById("roomSearch").value = "", c = getMap(a, b), "" !== c && addMap(c, a, b).then(function(b) {
...
function parseSearch(a) {
    ...
    for (var b = a.split(/[ \+-\s<>'"();\/\\]+/), c = "", d = "", e = "", f = 0; f < b.length; f++) 1 === b[f].length && isLetter(b[f]) && 0 === f ? c = b[f] : 4 === b[f].length ? isNumeric(b[f]) ? "2000" == b[f] || "3000" == b[f] ? c = b[f][0] : d = b[f] : "G" === b[f].charAt(0) && isNumeric(b[f].substring(1)) ? (d = b[f],
...
var building3 = [ "Building/3000/3000-Parking-Lot.html", "Building/3000/First-Floor.html", "Building/3000/Second-Floor.html", "Building/3000/Third-Floor.html", "Building/3000/Fourth-Floor.html" ];
var buildingA = [ "Building/A/A-Parking-Lot.html", "Building/A/First-Floor.html" ];
var buildingB = [ "Building/B/B-Parking-Lot.html","Building/B/first-floor.html", "Building/B/second-floor.html", "Building/B/third-floor.html" ];
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    activatePopup(d, "search-popup"), urlRoom(), loadSearch("Building\\AllRooms.txt"),
    window.addEventListener("click", function(a) {
        document.getElementById("roomSearch").contains(a.target) || document.getElementById("roomSearch").blur();
