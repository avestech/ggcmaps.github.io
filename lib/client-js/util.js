...
function parseSearch(str) {
  ...
  for (var i = 0; i < res.length; i++) {
    ...
    //If res[i] is 5 characters long
      ...
      if(isNumeric(res[i].substring(1))) {//if search is b1200
        room = res[i].substring(1);
        building = res[i].charAt(0);//added to set building to first char
      }
      else if(isNumeric(res[i].substring(0, 4))) {//searches like b 3500b
        room = res[i];
      }
    }

    //code for rooms with letter at end ie B3500b
    else if(res[i].length === 6 && isNumeric(res[i].substring(1, 5))) {
      room = res[i].substring(1);
      building = res[i].charAt(0);
      //roomNum = res[i].substring(1);
      //alert("building: " + building + " room: " + room + " roomNum: " + roomNum);
    }
