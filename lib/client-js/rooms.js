...
function searchRoomNumber() {
  ...
  if (rInfo.roomName !== '') { // Search by names instead of numbers
    ...
    if (roomNum === '' && !(rInfo.roomName in roomNames)) { // If no room name or number is found alert the user.
      alert(rInfo.roomName + ":" + rInfo.roomNum + ' is an invalid room name/number. Please check your spelling.');//changed from rInfo.roomNum to also include .roomName
    }
  }
  else {
    ...
    //as long as room is provided else only show building letter in url
    if(roomNum !== "4983") {
