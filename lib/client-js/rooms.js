    ...
    if (room !== '' && !room.includes('.') && room.toLowerCase() !== 'campus') { // If room is set and not campus or have a '.'
      switch (room.toUpperCase()) { // If room is a building letter use changeFloor to lowest floor of building
        ...
        default: // If not a building letter search for the room
          console.log("urlRoom calls assignBuilding");
          assignBuilding();
      }
...
// Search for a room
function searchRoomNumber(sRoom) {
  //******************************CHANGES!!!!!!*********************************
  closeMenu();
  ...
  //var sRoom = document.getElementById('roomSearch').value.toUpperCase();
    roomNum = "4983";
    building = document.getElementById('building').innerHTML; // Current Building
    curBuild = building;
    floor = document.getElementById('floor').innerHTML; // Current Floor

  // Empty  variables to compare the search to the current
    newBuilding = '';
    newFloor = '';
  //var rInfo = parseSearch(sRoom);
  // Assign variables from search
  if (search.building !== '') {
  ...
  if (search.roomName !== '') { // Search by names instead of numbers
    roomNum = '';
    ...
    for (var rmBuilding in roomNames) {
      ...
      for (var rmFloor in roomNames[rmBuilding]) {
        ...
        for (var rmRoom in roomNames[rmBuilding][rmFloor]) {
          ...
          if (tempRoom.name.toLowerCase() === search.roomName.toLowerCase()) {
    ...
    window.location.hash = search.roomName;

    if (roomNum === '') { // If no room is found alert the user
      alert('Were you trying to search by nickname? Unfortunately, "' + search.roomName + '" is not recognized.\nPlease check your spelling, or try a more common name.\n\nExample: Search for "Moes" not "Mo"');
    }
  }
  else {
    ...
    console.log("searchRoomNumber calls upperHash");
    upperHash();
  }
}
...
function assignBuilding() {
  ...
  // If either the building or the floor are different then a different map has to be loaded
  if (curBuild !== newBuilding || floor !== newFloor) {
    searchNewFloor(newBuilding, newFloor, roomNum);
  }
  else { // Otherwise activate the room
    console.log("assignBuilding calls activateRoom");
