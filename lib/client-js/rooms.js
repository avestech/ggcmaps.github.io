...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (window.location.hash !== '') {
    var roomNum = getHashRoomNum();
    history.pushState({room: roomNum}, roomNum, '/');
    urlRoom();
  }
  else {
    history.pushState({room: 'Campus'}, 'Campus');
    urlRoom();
  }
...
// Push a new state to the history with the data of roomNum
function pushRoomState(roomNum) {
  // Don't push a repeat state
  if (history.state !== null) {
    if (history.state.room !== roomNum) {
      history.pushState({room: roomNum}, roomNum);
    }
  }
  else {
...
function searchRoomNumber() {
  ...
  if (rInfo.roomName !== '') { // Search by names instead of numbers
    ...
    pushRoomState(building.toUpperCase() + roomNum.toUpperCase());
  }
