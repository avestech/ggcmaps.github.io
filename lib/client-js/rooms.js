...
function floorDropDown(building) {
    ...
    for (var b = 0; b < campus.buildings.length; b++) {
        if (building.toUpperCase() === campus.buildings[b].id.toUpperCase()) {
            floors = campus.buildings[b].floors;
        }
    }

    for (var i = 0; i < floors.length; i++) { // Create a element for each floor and add it to the dropdown
        var option = '<h2 onclick="changeFloor(\'' + building + '\', \'' + floors[i].id + '\')">Floor ' + floors[i].id + '</h2>';
...
//add highlight room change search to call this this function, in activate room call this function to show the new function pop-up
