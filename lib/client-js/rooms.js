...
function searchRoomNumber() {
  ...
  if (rInfo.roomName !== '') { // Search by names instead of numbers
    ...
    if (roomName === '' && roomNum === '') { // If no room is found alert the user
      alert(rInfo.roomName + ' is an invalid room name/number. Please check your spelling.');//changed from rInfo.roomNum to .roomName
    }
  }
  else {
