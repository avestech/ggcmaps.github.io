    ...
    if(eventDay >= date.getDate() && eventDay <= date.getDate() + 14) {
        ...
        // Checks the location of the event by referencing it with the JSON file
          ...
          if(isTrue == true) {
            ...
            if(location.length > 1 && location[1].includes("Building") == true) {
              var link = document.createElement("a");
...
function checkLocalStorage() {
  ...
  if(savedEvents[0] == null || savedEventNumber == 0) {
    getEvents();
  }
  else if(savedEvents[0][0].includes(date.toLocaleString("en", {month: "long"})) == false) {
    getEvents();
  }
  else{
