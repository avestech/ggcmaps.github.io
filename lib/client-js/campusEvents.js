    ...
    request.onreadystatechange = function() {
        if (request.readyState == 4) {
            ...
            for (const element of events) {
                if (element.className != "SECalendarSummaryDate") {
                    debug.msg(false);
                } else {
                    if (element.querySelectorAll('.SECalendarPropertyContainer').length > 1) { // If the number of events in the property container are greater than 1 then it does the for loop to iterate each event
                        for (var x = 0; x < element.querySelectorAll('.SECalendarPropertyContainer').length; x++) { // For loop to iterate each event based on the length of the property container. A property container should have the information for a single event.
                            container = element.querySelectorAll('.SECalendarSummaryDateEventList'); // Container to hold the SECalendarSummaryDateEventList which can hold even or odd SECalenderProperty classes
                            eventArray[eventNumber] = new Array(element.querySelector('h3').innerText, // Creates a new array and gets the date, event name, event time, and event location of the event at index i and at the property child at the x'th position
                        ...
                        container = element.querySelectorAll('.SECalendarPropertyContainer'); // Entire div block that has the class name SECalendarPropertContainer. Used for getting certain children at a certain index
                        eventArray[eventNumber] = new Array(element.querySelector('h3').innerText, // Adds the date to the eventArray by using querySelector to get the inner text of the h3 tag
                            element.querySelector('.SECalendarEventName').innerText, // Adds the event name to the eventArray by using querySelector to get the inner text of the class SECalendarEventName
    ...
    for (var i = 0; i < eventNumber; i++) {
        ...
        if (eventDay >= date.getDate() && eventDay <= date.getDate() + 14) {
            ...
            // Checks the location of the event by referencing it with the JSON file
                ...
                var nameFromJSON = '';
                var letter;
                for (const element of campus.buildings) {
                    for (var f = 0; f < element.floors.length; f++) {
                        for (let n = 0; n < element.floors[f].roomNames.length; n++) {
                            if (splitLocations.length >= 2 && element.floors[f].roomNames[n].name.toLowerCase().includes(splitLocations[0].toLowerCase() + "-" + splitLocations[1].toLowerCase()) == true) {
                                isTrue = true;
                                nameFromJSON = element.floors[f].roomNames[n].name;
                                letter = element.id.toUpperCase();
                                break;
                            }
                            else if (splitLocations.length == 1 && element.floors[f].roomNames[n].name.toLowerCase().includes(splitLocations[0].toLowerCase()) == true) {
                                debug.msg('test');
                                isTrue = true;
                                nameFromJSON = element.floors[f].roomNames[n].name;
                                letter = element.id.toUpperCase();
...
function sortEvents() {
    ...
    if (window.location.hash.length <= 2) {
        ...
        if (window.location.hash.includes("CG") == true || window.location.hash.includes("C") == true && window.location.hash.substr(2, 4) > 1300 && window.location.hash.substr(2, 4) < 1400 || window.location.hash.includes("C") == true && window.location.hash.substr(2, 4) > 2300 && window.location.hash.substr(2, 4) < 2400) {
            for (i = 0; i < listOfEvents.length; i++) {
    ...
    // If hash is a string of words or a combination
        ...
        for (const element of campus.buildings) {
            for (const floor of element.floors) {
                for (const roomName of floor.roomNames) {
                    if (window.location.hash.substr(1) === roomName.name) {
                        letter = element.id;
                        debug.msg(letter);
                    }
                }
