            ...
            // Checks the location of the event by referencing it with the JSON file
                ...
                for (const building of campus.buildings) {
                    for (const floor of building.floors) {
                        for (const room of floor.roomNames) {
                            if (splitLocations.length >= 2 && room.name.toLowerCase().includes(splitLocations[0].toLowerCase() + "-" + splitLocations[1].toLowerCase()) == true) {
                                isTrue = true;
                                nameFromJSON = room.name;
                                letter = building.id.toUpperCase();
                                break;
                            }
                            else if (splitLocations.length == 1 && room.name.toLowerCase().includes(splitLocations[0].toLowerCase()) == true) {
                                debug.msg('test');
                                isTrue = true;
                                nameFromJSON = room.name;
                                letter = building.id.toUpperCase();
...
function sortEvents() {
    ...
    // If hash is a string of words or a combination
        ...
        for (const building of campus.buildings) {
            for (const floor of building.floors) {
                for (const room of floor.roomNames) {
                    if (window.location.hash.substr(1) === room.name) {
                        letter = building.id;
                        debug.msg(letter);
                    }
                }
